...
// TODO: remove all 'any' from this file
    ...
    <Tabs defaultValue="event" className="flex flex-col h-full">
      ...
      <TabsContent value="state" className="flex-1 overflow-auto">
        {isLoading ? (
          ...
          <AgentReducedState reducedState={reducedState as never} className="h-full" />
        ) : (
...
export function AgentDetailRenderer({
  ...
  const handleBraintrustClick = async () => {
    if (getters.getBraintrustPermalink) {
          ...
          {events.length > 0 && (
            <div className="flex-shrink-0 px-4 py-2 border-b">
              <FilterBar
                ...
                onBrainClick={getters.getBraintrustPermalink ? handleBraintrustClick : undefined}
              />
            </div>
          )}
