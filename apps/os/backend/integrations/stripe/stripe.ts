...
/**
 * Makes a JSON POST request to the Stripe v2 API
 */
async function stripeV2JsonPost<T = unknown>(path: string, payload: unknown): Promise<T> {
...
/**
 * Makes a JSON GET request to the Stripe v2 API
 */
async function stripeV2JsonGet<T = unknown>(path: string): Promise<T> {
  const response = await fetch(`https://api.stripe.com${path}`, {
    method: "GET",
    headers: {
      Authorization: `Bearer ${env.STRIPE_SECRET_KEY}`,
...
export async function subscribeCustomerToPricingPlan(
  ...
  const plan = await stripeV2JsonGet<{ latest_version: string }>(
  ...
  const billingProfile = await stripeV2JsonPost<{ id: string }>(
    "/v2/billing/profiles",
  ...
  const cadence = await stripeV2JsonPost<{ id: string }>("/v2/billing/cadences", {
    payer: {
  ...
  const billingIntent = await stripeV2JsonPost<{ id: string }>("/v2/billing/intents", {
    cadence: cadence.id,
    currency: "usd",
    actions: [
      {
  ...
  const reserved = await stripeV2JsonPost<{ amount_details: { total: number } }>(
  ...
  if (paymentMethod) {
    ...
    committed = await stripeV2JsonPost(`/v2/billing/intents/${billingIntent.id}/commit`, {
...
export async function trackTokenUsageInStripe(params: {
  ...
  logger.debug("Tracking token usage:", { stripeCustomerId, model, inputTokens, outputTokens });

  const results = await Promise.all([
    fetch("https://api.stripe.com/v2/billing/meter_events", {
      method: "POST",
      headers: {
        Authorization: `Bearer ${env.STRIPE_SECRET_KEY}`,
        "Stripe-Version": V2_STRIPE_VERSION,
  ...
  for (const [i, res] of results.entries()) {
    try {
      ...
      logger.info(`Stripe meter event result ${i}:`, json);
    } catch (err) {
      logger.error(`Stripe meter event result ${i} error:`, err);
    }
  }
}
